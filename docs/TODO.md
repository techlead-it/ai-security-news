...
- [ ] [RED] dedupキー生成（url優先・無ければguid）のテスト作成
- [ ] [GREEN] dedupキー生成の実装
- [ ] [RED] 一次関連性キーワードフィルタ（AIセキュリティ判定）のテスト作成（該当/非該当/境界）
- [ ] [GREEN] キーワードフィルタの実装
- [ ] [RED] ラベル正規化（既存ラベル一覧を渡し表記揺れを既存名に寄せる／新規は採用）のテスト作成
- [ ] [GREEN] ラベル正規化の実装
- [ ] [RED] 要約・分類プロンプト組立（記事本文＋既存ラベルから AI 入力を生成）のテスト作成
- [ ] [GREEN] プロンプト組立の実装
- [ ] [RED] API レスポンス整形（記事→DTO、ラベル/カテゴリ埋め込み）のテスト作成
- [ ] [GREEN] レスポンス整形の実装
- [ ] [REFACTOR] 重複排除・命名整理・型名の明確化
- [ ] [REVIEW] フェーズ実装の簡易セルフレビューと修正
- [ ] [CHECK] `vp test` / `vp check --no-lint --no-fmt` / `vp build` の実行と確認

### フェーズ3: RSS取得・パース層

- [ ] [RED] フィードパース（タイトル・URL・公開日・抜粋・GUID 抽出）のテスト作成（RSS/Atom フィクスチャ）
- [ ] [GREEN] フィードパースの実装
- [ ] [RED] フィード取得の一時失敗時にスキップして他フィード処理を継続するテスト作成（HTTPクライアントはフェイク注入）
- [ ] [GREEN] フィード取得（インターフェース化したHTTPクライアント経由）の実装
- [ ] [RED] フィード定義（対象ソース一覧＋arXiv cs.CRのキーワード絞り込み）の読み込みテスト作成
- [ ] [GREEN] フィード定義の実装
- [ ] [REFACTOR] 取得層の整理
- [ ] [REVIEW] フェーズ実装の簡易セルフレビューと修正
- [ ] [CHECK] `vp test` / `vp check --no-lint --no-fmt` / `vp build` の実行と確認
...
- [ ] [RED] AIエンジンのインターフェース契約テスト作成（要約/詳細生成・関連性二次判定・ラベル分類）。テストはフェイクで固定出力
- [ ] [GREEN] AIエンジンのインターフェースとフェイク実装
- [ ] [RED] Workers AI 実装が入力プロンプトを正しく組み立て・出力をパースするテスト作成（AIバインディングはモック）
- [ ] [GREEN] Workers AI 実装（既定 Llama 系モデル。モデルIDは実装時に最新確認）
- [ ] [RED] 二次関連性判定で非AIセキュリティ記事を除外するテスト作成
- [ ] [GREEN] 二次関連性判定の実装
- [ ] [REFACTOR] エンジン抽象境界の整理（将来 Claude API 実装を差し替え可能に）
- [ ] [REVIEW] フェーズ実装の簡易セルフレビューと修正
- [ ] [CHECK] `vp test` / `vp check --no-lint --no-fmt` / `vp build` の実行と確認
...
- [ ] [RED] パイプライン全体フロー（取得→新着抽出→一次フィルタ→本文取得→二次判定→要約→ラベル分類→永続化）のテスト作成（全I/Oをフェイク注入）
- [ ] [GREEN] パイプライン統合の実装
- [ ] [RED] 1tickの処理件数キャップ（上限N件、残りは次回）のテスト作成 ※Nの初期値を決定
- [ ] [GREEN] 件数キャップの実装
- [ ] [RED] 記事単位の失敗分離（1記事のAI/取得失敗が全体を止めない）のテスト作成
- [ ] [GREEN] 失敗分離の実装
- [ ] [RED] Neuron日次上限到達時に処理済み分をコミットして正常終了するテスト作成
- [ ] [GREEN] 上限到達ハンドリングの実装
- [ ] [RED] AI一時エラーのリトライ（指数バックオフ）後スキップのテスト作成
- [ ] [GREEN] リトライの実装
- [ ] [RED] 収集サマリログ（取得/新規/除外/取得失敗/AIエラー件数）出力のテスト作成
- [ ] [GREEN] サマリログの実装
- [ ] [GREEN] Cron handler / 手動トリガーから本パイプラインを起動
- [ ] [REFACTOR] パイプライン構成の整理
- [ ] [REVIEW] フェーズ実装の簡易セルフレビューと修正
- [ ] [CHECK] `vp test` / `vp check --no-lint --no-fmt` / `vp build` の実行と確認
...
- [ ] [RED] 記事一覧コンポーネント（新着順・タイトル/ソース/公開日/要約/ラベル表示）のレンダリングテスト作成
- [ ] [GREEN] 一覧コンポーネントの実装（API取得はデータソース注入）
- [ ] [RED] ラベルクリックで絞り込み遷移するインタラクションテスト作成
- [ ] [GREEN] ラベル絞り込み（ルーティング）の実装
- [ ] [RED] 検索ボックスで日本語キーワード検索するテスト作成
- [ ] [GREEN] 検索UIの実装
- [ ] [RED] 記事詳細（要約・詳細・元リンク・メタ・取得失敗フラグ表示）のテスト作成
- [ ] [GREEN] 記事詳細の実装
- [ ] [RED] 空状態・ローディング・エラー表示のエッジケーステスト作成
- [ ] [GREEN] エッジケース対応
- [ ] [REFACTOR] スタイルとロジックの分離・コンポーネント整理
- [ ] [REVIEW] フェーズ実装の簡易セルフレビューと修正
- [ ] [CHECK] `vp test` / `vp check --no-lint --no-fmt` / `vp build` の実行と確認
...
- [ ] D1 マイグレーション適用を CI もしくは運用手順として整備（デプロイ時にスキーマ適用）
- [ ] Cron Trigger 設定がデプロイに反映されることを確認
- [ ] README にセットアップ・運用手順（シークレット・マイグレーション・手動トリガー）を記載
- [ ] [REVIEW] フェーズ実装の簡易セルフレビューと修正
- [ ] [CHECK] `vp test` / `vp check --no-lint --no-fmt` / `vp build` の実行と確認
...
### 未確定で実装着手時に決める値
- 1tickあたりの処理上限 N（初期目安 10〜15。サブリクエスト~50/CPU制約から逆算して確定）
- Workers AI のモデルID（実装時に利用可能な最新 Llama instruct 系を確認）
- 対象RSSフィードURLの到達性検証結果（提供有無でソースを取捨）
